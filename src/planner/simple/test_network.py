...
from gtfs_flex.object import Trip as flex_Trip, Stop as flex_Stop, StopTime as flex_StopTime, Service as flex_Service, \
    Group as flex_Group
...
class WalkingTestCase(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.network = WalkingNetwork("walking", walking_meters_per_minute=self.walking_velocity)
...
class GbfsTestCase(unittest.TestCase):
    ...
    def test_successfully_created_graph(self):
        ...
        self.assertEqual(locations_length * (locations_length - 1), len(self.network.graph.edges))
    ...
    def test_correct_path_case(self):
        ...
        expected = Path([
            Trip(
                ...
                dept=10,
                arrv=10 + org.distance(self.stations["2_1"]) / self.walking_velocity,
                service="walking"
            ),
            Trip(
                org=self.stations["2_1"],
                dst=self.stations["22_1"],
                dept=10 + org.distance(self.stations["2_1"]) / self.walking_velocity,
                arrv=10 + org.distance(self.stations["2_1"]) / self.walking_velocity
                + self.stations["2_1"].distance(self.stations["22_1"]) / self.mobility_velocity,
                service=self.service_name
            ),
            Trip(
...
class GtfsTestCase(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.network = GtfsNetwork(
            service=self.service_name,
            walking_meters_per_minute=self.walking_velocity,
            start_time=datetime.datetime.combine(datetime.date.today(), datetime.time())
        )
    ...
    def test_correct_path_case(self):
        ...
        expected = Path([
            Trip(
                ...
                dept=530.0,
                arrv=530.0 + org.distance(self.stations["3_1"]) / self.walking_velocity,
                service="walking"
            ), Trip(
                ...
                dst=dst,
                dept=548.0,
                arrv=548 + self.stations["7_1"].distance(dst) / self.walking_velocity,
                service="walking"
            )
        ])
...
class OndemandBusTestCase(unittest.TestCase):
    ...
    def test_no_path_case_not_available(self):
        ...
        self.assertEqual(
            Path(trips=[Trip(org=org, dst=dst, dept=dept, arrv=float('inf'), service="not_found")]),
            path
        )
    ...
    def test_correct_path_case(self):
        ...
        expected = Path([
            Trip(
                ...
                dept=dept,
                arrv=dept + org.distance(self.stations["36_1"]) / self.walking_velocity,
                service="walking"
            ), Trip(
                org=self.stations["36_1"],
                dst=self.stations["20_1"],
                dept=dept + org.distance(self.stations["36_1"]) / self.walking_velocity,
                arrv=dept + org.distance(self.stations["36_1"]) / self.walking_velocity
                + self.stations["36_1"].distance(self.stations["20_1"]) / self.mobility_velocity
